---
title: Configuration
description: SynapseAI App is configured through two files and a set of environment variables. This page covers the application-level configuration that most operators need to set up before deploying.
---
...
SynapseAI App is configured through two files and a set of environment variables. This page covers the application-level configuration that most operators need to set up before deploying.
...
models:
  - name: deepseek-v4
    use: synapse.models.patched_deepseek:PatchedChatDeepSeek
...
Install Ollama provider: `cd backend && uv add 'synapse-harness[ollama]'`
...
```yaml
sandbox:
  use: synapse.sandbox.local:LocalSandboxProvider
...
```yaml
sandbox:
  use: synapse.community.aio_sandbox:AioSandboxProvider
  image: enterprise-public-cn-beijing.cr.volces.com/vefaas-public/all-in-one-sandbox:latest
  replicas: 3
  idle_timeout: 600
```

Install: `cd backend && uv add 'synapse-harness[aio-sandbox]'`
...
tools:
  ...
  - use: synapse.community.ddg_search.tools:web_search_tool # default, no key required
  # - use: synapse.community.tavily.tools:web_search_tool
  #   api_key: $TAVILY_API_KEY

  # Web fetch (choose one)
  - use: synapse.community.jina_ai.tools:web_fetch_tool

  # Image search
  - use: synapse.community.image_search.tools:image_search_tool

  # File operations
  - use: synapse.sandbox.tools:ls_tool
  - use: synapse.sandbox.tools:read_file_tool
  - use: synapse.sandbox.tools:glob_tool
  - use: synapse.sandbox.tools:grep_tool
  - use: synapse.sandbox.tools:write_file_tool
  - use: synapse.sandbox.tools:str_replace_tool
  - use: synapse.sandbox.tools:bash_tool
```

### Database backend

SynapseAI uses the `database` section for both LangGraph checkpoint data and application data such as runs, feedback, and thread metadata.

By default, SynapseAI uses SQLite for local, single-node persistence:
...
SQLite mode stores everything in one `synapse.db` file. This is fine for development or single-user deployments, but concurrent production traffic can hit SQLite's single-writer limit and raise `sqlite3.OperationalError: database is locked`.
...
Set `DATABASE_URL` in your environment, for example `postgresql://user:password@localhost:5432/synapse`.
...
memory:
  ...
  backend_config:                  # DeerMem-private knobs (the backend self-interprets these)
    storage_path: ""               # empty = synapse-ai base_dir (per-user memory under it)
    debounce_seconds: 30
    max_facts: 100
...
through `langchain-openviking` and currently supports one SynapseAI user in
