...
description: SynapseAI App supports multiple named agents and maintains conversation state across sessions through threads and checkpointing.
---
...
SynapseAI App supports multiple named agents and maintains conversation state across sessions through threads and checkpointing.
...
```
backend/.synapse-ai/threads/{thread_id}/user-data/
```

This directory is mounted into the sandbox as `/mnt/user-data/` for the agent to read and write.

In Docker deployments, this path is bind-mounted from the host so that thread data persists across container restarts. Set `SYNAPSE_ROOT` to the absolute path of your synapse-ai repository root to ensure the correct host path is used.
