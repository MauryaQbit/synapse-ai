...
description: "This guide covers all supported deployment methods for SynapseAI App: local development, Docker Compose, and production with Kubernetes-managed sandboxes."
---
...
This guide covers all supported deployment methods for SynapseAI App: local development, Docker Compose, and production with Kubernetes-managed sandboxes.

## Local development deployment

The local workflow is the fastest way to run SynapseAI. All services run as native processes on your machine.
...
```bash
# Set the absolute path to your synapse-ai repo root
export SYNAPSE_ROOT=/path/to/synapse-ai
...
# .env
OPENAI_API_KEY=sk-...
SYNAPSE_ROOT=/absolute/path/to/synapse-ai
...
Thread data is stored in `backend/.synapse-ai/threads/`. In Docker deployments, this directory is bind-mounted into the gateway container.

To avoid data loss when containers are recreated:

1. Set `SYNAPSE_ROOT` to the absolute repo root path (or a stable host path).
...
In SQLite mode, SynapseAI stores LangGraph checkpoint data and application data in one `synapse.db` file. WAL mode allows concurrent reads, but SQLite still permits only one writer at a time. When several users run agents at once, checkpoint writes and application writes can contend for the same file and raise `sqlite3.OperationalError: database is locked`.

For production or any multi-user deployment, use Postgres:

```bash
# .env
DATABASE_URL=postgresql://user:password@postgres:5432/synapse
UV_EXTRAS=postgres
```
...
USERDATA_PVC_NAME=synapse-ai-userdata-pvc
SKILLS_PVC_NAME=synapse-ai-skills-pvc
```

When `USERDATA_PVC_NAME` is set, the provisioner automatically uses subPath (`synapse-ai/users/{user_id}/threads/{thread_id}/user-data`) so each user's threads get their own directories in the PVC.
...
SynapseAI App uses [Better Auth](https://www.better-auth.com/) for session management. In production:
