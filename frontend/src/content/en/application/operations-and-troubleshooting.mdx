...
description: This page covers day-to-day operational tasks and solutions to common problems when running SynapseAI App.
---
...
This page covers day-to-day operational tasks and solutions to common problems when running SynapseAI App.
...
When you pull a new version of SynapseAI, the `config_version` in `config.example.yaml` may be higher than your `config.yaml`. To merge new fields without losing your customizations:
...
This usually means SQLite is handling concurrent writes from multiple users or long-running agent runs. SQLite supports concurrent reads, but only one writer can hold the database lock at a time. In SynapseAI, checkpoint writes and application writes share the same SQLite database file.
...
- Check that the storage path is writable: `ls -la backend/.synapse-ai/`.
...
- `backend/.synapse-ai/threads/` for thread working directories, uploads, and artifacts.
- `backend/.synapse-ai/memory.json` and `backend/.synapse-ai/agents/` for learned memory.
- `${database.sqlite_dir}/synapse.db` when using `database.backend: sqlite` (default: `.synapse-ai/data/synapse.db`).
