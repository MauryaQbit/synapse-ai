...
description: Why tool calls left in a safety-terminated model response must not be executed, and how to configure provider detectors in SynapseAI.
date: 2026-05-22
tags:
  - Safety
  - Agents
...
[the implementation commit](https://github.com/MauryaQbit/synapse-ai/commit/be0eae9825619b63ca0c67b253d40b5eee76a2d6) addresses this boundary: when a provider stops generation with a safety signal while the response still contains tool calls, SynapseAI should suppress those tool calls first and record the turn as a safety termination event.
...
## What SynapseAI Does at This Boundary
...
When `detectors` is not configured explicitly, SynapseAI uses the built-in detector set:
...
This default set covers common SynapseAI paths for OpenAI-compatible providers, Anthropic, and Gemini. It does not treat a normal `finish_reason="tool_calls"` as a safety termination, and it does not fold length truncation such as `length` or `max_tokens` into the safety category.
...
safety_finish_reason:
  enabled: true
  detectors:
    - use: synapse.agents.middlewares.safety_termination_detectors:OpenAICompatibleContentFilterDetector
      config:
        finish_reasons: ["content_filter", "sensitive"]

    - use: synapse.agents.middlewares.safety_termination_detectors:AnthropicRefusalDetector

    - use: synapse.agents.middlewares.safety_termination_detectors:GeminiSafetyDetector
```
