---
title: Customization
description: SynapseAI's pluggable architecture means most parts of the system can be replaced or extended without forking the core. This page maps the extension points and explains how to use each one.
---
...
<Callout type="info" emoji="🔧">
  SynapseAI is designed to be adapted. You can extend agent behavior by writing
...
SynapseAI's pluggable architecture means most parts of the system can be replaced or extended without forking the core. This page maps the extension points and explains how to use each one.
...
from synapse.agents.thread_state import ThreadState
...
```python
from synapse.sandbox.sandbox_provider import SandboxProvider
from synapse.sandbox.sandbox import Sandbox
...
```python
from synapse.agents.memory.storage import MemoryStorage
...
For custom backends, implement the LangGraph `BaseCheckpointSaver` interface and configure it programmatically when initializing the `SynapseAIClient`.
...
guardrails:
  enabled: true
  provider:
    use: synapse.guardrails.builtin:AllowlistProvider
    config:
