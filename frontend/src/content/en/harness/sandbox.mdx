...
<Callout type="info" emoji="📦">
  ...
  command-based work. It is what makes SynapseAI capable of real action, not just
  conversation.
</Callout>
...
SynapseAI supports four sandbox modes. Choose the one that fits your deployment:
...
sandbox:
  ...
  # Optional: container name prefix (default: synapse-ai-sandbox)
...
```yaml
sandbox:
  use: synapse.community.e2b_sandbox:E2BSandboxProvider
...
Install: `e2b-code-interpreter` is already a core dependency of `synapse-harness`, so no extra install step is needed — just provide your API key and switch the provider in `config.yaml`.
...
<Callout type="info">
  Because e2b is a remote sandbox, SynapseAI's `/mnt/user-data` virtual prefix is
...
| `.synapse-ai/threads/{thread_id}/user-data/` | `/mnt/user-data/`                            | Read-write |
...
provider runs inside the `synapse-ai-gateway` container. In that mode,
...
If the configured `host_path` is not visible to the gateway process, SynapseAI
