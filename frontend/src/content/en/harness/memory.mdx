---
title: Memory
description: Memory is a runtime feature of the SynapseAI Harness. It is not a simple conversation log — it is a structured store of facts and context summaries that persist across separate sessions and inform the agent's behavior in future conversations.
---
...
<Callout type="info" emoji="💾">
  Memory lets SynapseAI carry useful information across sessions. The agent
...
Memory is a runtime feature of the SynapseAI Harness. It is not a simple conversation log — it is a structured store of facts and context summaries that persist across separate sessions and inform the agent's behavior in future conversations.
...
memory:
  ...
  backend_config:
    # Data root. Empty = synapse-ai base_dir; per-user memory at
...
`langchain-openviking` adapter. This first version supports one SynapseAI user
...
Use `owner_user_id: default` when SynapseAI authentication is disabled. Put the
...
SynapseAI supports two levels of memory:
...
- Base directory: `backend/.synapse-ai/`
- Global memory: `backend/.synapse-ai/memory.json`
- Per-agent memory: `backend/.synapse-ai/agents/{agent_name}/memory.json`
