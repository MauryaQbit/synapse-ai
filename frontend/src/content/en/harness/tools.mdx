...
<Callout type="info" emoji="🔧">
  Tools are the actions the Lead Agent can take. SynapseAI provides built-in
...
SynapseAI organizes tools into four categories:
...
tools:
  ...
  - use: synapse.sandbox.tools:bash_tool # requires host bash or container sandbox
```
...
```yaml
tools:
  - use: synapse.community.ddg_search.tools:web_search_tool
```
...
Install: `cd backend && uv add 'synapse-harness[tavily]'`

  </Tabs.Tab>
  <Tabs.Tab>
```yaml
tools:
  - use: synapse.community.brave.tools:web_search_tool
...
```yaml
tools:
  - use: synapse.community.exa.tools:web_search_tool
    api_key: $EXA_API_KEY
```
Semantic search with neural retrieval. Requires an [Exa](https://exa.ai) API key.

Install: `cd backend && uv add 'synapse-harness[exa]'`

  </Tabs.Tab>
  <Tabs.Tab>
```yaml
tools:
  - use: synapse.community.infoquest.tools:web_search_tool
...
```yaml
tools:
  - use: synapse.community.firecrawl.tools:web_search_tool
    api_key: $FIRECRAWL_API_KEY
```
Firecrawl-powered search and crawl. Requires a [Firecrawl](https://firecrawl.dev) API key.

Install: `cd backend && uv add 'synapse-harness[firecrawl]'`

  </Tabs.Tab>
  <Tabs.Tab>
```yaml
tools:
  - use: synapse.community.groundroute.tools:web_search_tool
...
Install: `cd backend && uv add 'synapse-harness[groundroute]'`
...
tools:
  - name: web_fetch
    group: web
    use: synapse.community.browserless.tools:web_fetch_tool
...
```yaml
tools:
  - use: synapse.community.exa.tools:web_fetch_tool
    api_key: $EXA_API_KEY
```
  </Tabs.Tab>
  <Tabs.Tab>
```yaml
tools:
  - use: synapse.community.infoquest.tools:web_fetch_tool
    api_key: $INFOQUEST_API_KEY
```
  </Tabs.Tab>
  <Tabs.Tab>
```yaml
tools:
  - use: synapse.community.firecrawl.tools:web_fetch_tool
    api_key: $FIRECRAWL_API_KEY
```
  </Tabs.Tab>
  <Tabs.Tab>
```yaml
tools:
  - use: synapse.community.groundroute.tools:web_fetch_tool
    api_key: $GROUNDROUTE_API_KEY
```
  </Tabs.Tab>
</Tabs>
...
tools:
  - name: web_capture
    group: web
    use: synapse.community.browserless.tools:web_capture_tool
    base_url: http://localhost:3032
    # token: $BROWSERLESS_TOKEN # required for Browserless Cloud; omit for self-hosted (a $VAR that is unset fails startup)
    output_format: png
    full_page: true
...
through SynapseAI's artifact system. This is useful for visual evidence, UI checks,
...
```yaml
tools:
  - use: synapse.community.infoquest.tools:image_search_tool
    api_key: $INFOQUEST_API_KEY
```
...
```yaml
tools:
  - use: synapse.community.serper.tools:image_search_tool
...
```yaml
tools:
  - use: synapse.community.brave.tools:image_search_tool
    api_key: $BRAVE_SEARCH_API_KEY
```
