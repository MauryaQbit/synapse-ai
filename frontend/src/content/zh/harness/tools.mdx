...
<Callout type="info" emoji="🔧">
  工具是 Lead Agent 可以采取的行动。SynapseAI 提供内置工具、社区集成、MCP
...
安装：`cd backend && uv add 'synapse-harness[tavily]'`
...
安装：`cd backend && uv add 'synapse-harness[exa]'`
...
安装：`cd backend && uv add 'synapse-harness[groundroute]'`
...
使用 Browserless `/screenshot` 渲染网页并截图，然后通过 SynapseAI 的 artifact
