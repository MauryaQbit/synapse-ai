---
title: Agent 与线程
description: 了解 SynapseAI 中 Agent 与线程的关系，以及如何管理自定义 Agent 和对话线程。
---
...
SynapseAI 允许你创建多个具有不同专业领域的自定义 Agent。每个 Agent 使用 SynapseAI Harness 相同的 Lead Agent 运行时，但具有不同的：
...
SynapseAI 在每次 Agent 轮次后自动保存线程状态。使用持久化后端时，这允许：
...
每个线程在 `.synapse-ai/threads/{thread_id}/` 下有一个专用目录：

```
.synapse-ai/threads/{thread_id}/
  user-data/
    uploads/     ← 用户上传的文件
