---
title: 配置
description: 本页面涵盖 SynapseAI 应用的所有配置层——`config.yaml`、前端环境变量、`extensions_config.json` 和运行时环境变量。
---
...
本页面涵盖 SynapseAI 应用的所有配置层——`config.yaml`、前端环境变量、`extensions_config.json` 和运行时环境变量。

## config.yaml

`config.yaml` 是 SynapseAI 的主要配置文件。所有 Agent 行为、模型选择、工具加载和运行时功能都由它控制。
...
SynapseAI 使用 `database` 配置同时管理 LangGraph 检查点数据和应用数据，例如运行记录、反馈和线程元数据。
...
SQLite 模式会把数据写入同一个 `synapse.db` 文件。它适合开发或单用户部署，但生产并发流量可能触发 SQLite 的单写者限制，出现 `sqlite3.OperationalError: database is locked`。
...
在环境变量中设置 `DATABASE_URL`，例如 `postgresql://user:password@localhost:5432/synapse`。
...
<Callout type="warning">
  不要为 SynapseAI 工作区文件引入 MCP filesystem server。它会与 SynapseAI
  内置文件工具形成路径语义不同的重复能力，让 LLM 行为不稳定。SynapseAI
...
技能启用状态会反映在 `extensions_config.json` 中。你可以直接编辑它，或通过 SynapseAI 应用界面进行管理。

## 运行时环境变量

这些变量在 SynapseAI 进程中设置（通过 `.env`、Docker 环境或 shell）：

| 变量                    | 默认值           | 描述                                             |
| ----------------------- | ---------------- | ------------------------------------------------ |
| `SYNAPSE_CONFIG_PATH` | 自动发现         | `config.yaml` 的绝对路径                         |
| `LOG_LEVEL`             | `info`           | 日志详细程度（`debug`/`info`/`warning`/`error`） |
| `SYNAPSE_ROOT`        | 仓库根目录       | 用于 Docker 中的技能和线程挂载                   |
