---
title: 运维与排障
description: 本页面涵盖运行 SynapseAI 应用的操作信息：日志记录、常见问题和维护任务。
---
...
本页面涵盖运行 SynapseAI 应用的操作信息：日志记录、常见问题和维护任务。

## 日志

SynapseAI 应用在 `logs/` 目录中写入每个服务的日志：
...
```bash
# 检查线程用户数据目录是否存在且可写
ls -la backend/.synapse-ai/threads/
```

**解决**：

1. 确保 `backend/.synapse-ai/` 对运行 SynapseAI 的进程可写。
2. 在 Docker 部署中，验证卷挂载路径正确（`SYNAPSE_ROOT` 设置为绝对路径）。
...
**原因**：SQLite 支持并发读取，但同一时间只有一个写入者。SynapseAI 的检查点写入和应用写入会共享同一个 SQLite 数据库文件，多用户并发时可能竞争文件锁。
...
- `backend/.synapse-ai/threads/`：线程工作目录、上传文件和产出物。
- `backend/.synapse-ai/memory.json` 和 `backend/.synapse-ai/agents/`：全局和 Agent 记忆。
- `${database.sqlite_dir}/synapse.db`：当使用 `database.backend: sqlite` 时备份，默认路径为 `.synapse-ai/data/synapse.db`。
