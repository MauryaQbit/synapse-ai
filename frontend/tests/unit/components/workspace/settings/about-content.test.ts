...
test("aboutMarkdown heading interpolates the app version", async () => {
  ...
  expect(aboutMarkdown).toContain("[About SynapseAI 9.9.9-test]");
  // Milestone copy in the acknowledgments refers to the 1.0/2.0 product
  // generations and must NOT be parameterized.
  expect(aboutMarkdown).toContain("SynapseAI 1.0 and 2.0");
});
...
test("aboutMarkdown heading reflects the package version when env is unset", async () => {
  ...
  // empty or undefined APP_VERSION interpolation (`About SynapseAI ]` /
  // `About SynapseAI undefined]`), not just removal of the old literal.
  expect(aboutMarkdown).toContain(`[About SynapseAI ${APP_VERSION}]`);
});
