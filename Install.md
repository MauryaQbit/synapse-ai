...
Bootstrap a SynapseAI local development workspace on the user's machine with the least risky path available.
...
- The SynapseAI repository is cloned and the current working directory is the repo root.
- `config.yaml` exists.
...
- The user receives the exact next command to launch SynapseAI.
...
5. Exact next command to start SynapseAI
