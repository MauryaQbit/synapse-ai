...
> 28 февраля 2026 года SynapseAI занял 🏆 #1 в GitHub Trending после релиза версии 2. Спасибо огромное нашему сообществу — всё благодаря вам! 💪🔥

SynapseAI (**D**eep **E**xploration and **E**fficient **R**esearch **Flow**) — open-source **Super Agent Harness**, который управляет **Sub-Agents**, **Memory** и **Sandbox** для решения почти любой задачи. Всё на основе расширяемых **Skills**.
...
Больше информации и живые демо на [**официальном сайте**](https://SynapseAI.tech).
...
- [**LLM Space**](https://github.com/synapse-ai/llm-space) - Познакомьтесь с нашим секретным оружием за SynapseAI — настольный инструмент для прототипирования идей агентов, проверки каждого шага харнесса, воспроизведения сбоев и тестирования производительности.

## Coding Plan от ByteDance Volcengine

- Рекомендуем Doubao-Seed-2.0-Code, DeepSeek v3.2 и Kimi 2.5 для запуска SynapseAI
...
SynapseAI интегрирован с инструментарием для умного поиска и краулинга от BytePlus — [InfoQuest (есть бесплатный онлайн-доступ)](https://docs.byteplus.com/en/docs/InfoQuest/What_is_Info_Quest)
...
2. **Запустить мастер настройки (рекомендуется)**
   ...
   Из корня проекта (`synapse-ai/`) запустите:
   ...
   `.synapse-ai/support-bundles/`. Если issue оформляет AI-ассистент, он должен начать
   ...
   Для vLLM 0.19.0 используйте `SynapseAI.models.vllm_provider:VllmChatModel`. Для reasoning-моделей в стиле Qwen SynapseAI переключает режим рассуждений через `extra_body.chat_template_kwargs.enable_thinking` и сохраняет нестандартное поле `reasoning` vLLM в многоходовых диалогах с вызовами инструментов. Устаревшие конфигурации `thinking` автоматически нормализуются для обратной совместимости. Reasoning-моделям также может потребоваться запуск сервера с флагом `--reasoning-parser ...`. Если ваш локальный vLLM принимает любой непустой API-ключ, всё равно задайте `VLLM_API_KEY` со значением-заглушкой.
...
Предварительное условие: сначала выполните шаги раздела «Конфигурация» выше (`make setup`). Для `make dev` нужен корректный `config.yaml` в корне проекта. Задайте `SYNAPSE_PROJECT_ROOT`, чтобы явно указать корень проекта, или `SYNAPSE_CONFIG_PATH`, чтобы указать конкретный файл конфигурации. Состояние времени выполнения по умолчанию записывается в `.synapse-ai` в корне проекта и может быть перенесено через `SYNAPSE_HOME`; skills по умолчанию читаются из `skills/` в корне проекта, путь можно переопределить через `SYNAPSE_SKILLS_PATH`. Перед запуском выполните `make doctor`, чтобы проверить настройку.
...
SynapseAI поддерживает несколько режимов выполнения:
...
SynapseAI поддерживает настраиваемые MCP-серверы для расширения возможностей. Для HTTP/SSE MCP-серверов поддерживаются OAuth-токены (`client_credentials`, `refresh_token`). Подробнее в [руководстве по MCP-серверу](backend/docs/MCP_SERVER.md).

#### Мессенджеры

SynapseAI принимает задачи прямо из мессенджеров. Каналы запускаются автоматически при настройке, публичный IP не нужен.

SynapseAI может также предоставлять в workspace UI пользовательские подключения IM-каналов. Когда включён `channel_connections`, вошедшие в систему пользователи могут привязать Telegram, Slack, Discord, Feishu/Lark, DingTalk, WeChat или WeCom из боковой панели / Settings > Channels. Это переиспользует существующие исходящие транспорты `channels.*`, поэтому публичный IP или URL обратного вызова провайдера не требуются. Входящие IM-сообщения выполняются от имени подключённого пользователя SynapseAI. Настройки и вопросы безопасности описаны в [IM Channel Connections](backend/docs/IM_CHANNEL_CONNECTIONS.md).
...
4. После успешного прохождения QR-процесса SynapseAI сохраняет полученный токен в `state_dir` для последующих перезапусков.
...
> Сообщения без команды воспринимаются как обычный чат — SynapseAI создаёт тред и отвечает.

#### Трассировка LangSmith

SynapseAI имеет встроенную интеграцию с [LangSmith](https://smith.langchain.com) для наблюдаемости. При включении все вызовы LLM, запуски агентов и выполнения инструментов отслеживаются и отображаются в дашборде LangSmith.
...
SynapseAI также поддерживает наблюдаемость через [Langfuse](https://langfuse.com) для запусков, совместимых с LangChain.
...
- `tags` = `[env:<SYNAPSE_ENV>, model:<model_name>]` (опускается, если не заданы)
- `metadata.SynapseAI_trace_id` = идентификатор корреляции запросов SynapseAI, совпадающий с `X-Trace-Id`, когда корреляция трассировки запросов включена

Эти поля внедряются в `RunnableConfig.metadata` в корне вызова графа как для gateway-пути (`runtime/runs/worker.py::run_agent`), так и для встроенного пути (`client.py::SynapseAIClient.stream`), поэтому любой LangChain-совместимый callback может их прочитать. Установите `SYNAPSE_ENV` (или `ENVIRONMENT`) для тегирования трасс по среде развёртывания.

#### Использование обоих провайдеров

Если и LangSmith, и Langfuse включены, SynapseAI подключает оба callback'а трассировки и отправляет одну и ту же активность модели в обе системы.

Если провайдер явно включён, но отсутствуют необходимые учётные данные, или если его callback не может инициализироваться, SynapseAI завершает работу с ошибкой (fail fast) при инициализации трассировки во время создания модели, а сообщение об ошибке указывает провайдера, вызвавшего сбой.
...
SynapseAI начинался как фреймворк для Deep Research, и сообщество вышло далеко за эти рамки. После запуска разработчики строили пайплайны, генерировали презентации, поднимали дашборды, автоматизировали контент. То, чего мы не ожидали.

Стало понятно: SynapseAI не просто research-инструмент. Это **harness**: runtime, который даёт агентам необходимую инфраструктуру.

Поэтому мы переписали всё с нуля.

SynapseAI 2.0 — это Super Agent Harness «из коробки». Batteries included, полностью расширяемый. Построен на LangGraph и LangChain. По умолчанию есть всё, что нужно агенту: файловая система, memory, skills, sandbox-выполнение и возможность планировать и запускать sub-agents для сложных многошаговых задач.
...
Skills — это то, что позволяет SynapseAI делать почти что угодно.

Agent Skill — это структурированный модуль: Markdown-файл с описанием воркфлоу, лучших практик и ссылок на ресурсы. SynapseAI поставляется со встроенными skills для ресёрча, генерации отчётов, слайдов, веб-страниц, изображений и видео. Но главное — расширяемость: добавляйте свои skills, заменяйте встроенные или собирайте из них составные воркфлоу.
...
Skill `claude-to-SynapseAI` позволяет работать с SynapseAI прямо из [Claude Code](https://docs.anthropic.com/en/docs/claude-code). Отправляйте задачи, проверяйте статус, управляйте тредами, не выходя из терминала.
...
**Что можно делать**:
- Отправлять сообщения в SynapseAI и получать потоковые ответы
- Выбирать режимы выполнения: flash (быстро), standard, pro (planning), ultra (sub-agents)
- Проверять статус SynapseAI, просматривать модели, скиллы, агентов
- Управлять тредами и историей диалога
- Загружать файлы для анализа

Полный справочник API в [`skills/public/claude-to-SynapseAI/SKILL.md`](skills/public/claude-to-SynapseAI/SKILL.md).

### Цели сессии (Session Goals)

Используйте `/goal <условие завершения>`, чтобы привязать к текущему треду одно активное условие завершения. Цель — это состояние уровня треда, а не активация навыка, поэтому она остаётся активной между ходами, пока SynapseAI не сочтёт её выполненной или пока вы её не очистите.
...
После каждого запуска, выполненного через Gateway, SynapseAI оценивает видимый диалог относительно активной цели с помощью non-thinking модели-оценщика. Оценщик должен вернуть типизированный блокер (`missing_evidence`, `needs_user_input`, `run_failed`, `external_wait` или `goal_not_met_yet`) с видимыми доказательствами. SynapseAI добавляет hidden continuation только тогда, когда последний ход assistant сохранён в чекпоинте, блокер имеет тип `goal_not_met_yet`, тред не изменился во время оценки и счётчик отсутствия прогресса не сработал. Предел безопасности по умолчанию — 8 hidden continuation, а повторяющиеся одинаковые оценки без прогресса останавливаются после 2 попыток. `/goal clear` и любой новый ввод от пользователя имеют приоритет над continuation в очереди. Когда цель выполнена, SynapseAI очищает её автоматически и публикует обновлённое состояние треда.
...
Сложные задачи редко решаются за один проход. SynapseAI их декомпозирует.
...
Вот как SynapseAI справляется с задачами на минуты и часы: research-задача разветвляется в дюжину sub-agents, каждый копает свой угол, потом всё сходится в один отчёт, или сайт, или слайддек со сгенерированными визуалами. Один harness, много рук.

### Sandbox & файловая система

SynapseAI не просто *говорит* о том, что умеет что-то делать. У него есть собственный компьютер.
...
**Управление контекстом**: внутри сессии SynapseAI агрессивно сжимает контекст и суммирует завершённые подзадачи, выгружает промежуточные результаты в файловую систему, сжимает то, что уже не актуально. На длинных многошаговых задачах контекстное окно не переполняется.

### Long-Term Memory

Большинство агентов забывают всё, когда диалог заканчивается. SynapseAI помнит.

SynapseAI сохраняет ваш профиль, предпочтения и накопленные знания между сессиями. Чем больше используете, тем лучше он вас знает: стиль, технологический стек, повторяющиеся воркфлоу. Всё хранится локально и остаётся под вашим контролем.

## Рекомендуемые модели

SynapseAI работает с любым LLM через OpenAI-совместимый API. Лучше всего — с моделями, которые поддерживают:
...
SynapseAI можно использовать как Python-библиотеку прямо в коде — без запуска HTTP-сервисов. `SynapseAIClient` даёт доступ ко всем возможностям агента и Gateway, возвращает те же схемы ответов, что и HTTP Gateway API. HTTP Gateway также предоставляет `DELETE /api/threads/{thread_id}` для удаления локальных данных треда, управляемых SynapseAI, после того как сам LangGraph thread был удалён:
...
Теперь в SynapseAI есть первоклассный MVP запланированных задач (scheduled-task) в workspace.
...
- Фоновые запланированные запуски выполняются как неинтерактивные запуски SynapseAI (`ask_clarification` там не предоставляется)
...
- Запланированные задачи выполняются через стандартный жизненный цикл запуска SynapseAI
...
`SynapseAI` — это нативная терминальная панель для тех, кто живёт в шелле. Она работает **встроенной** поверх `SynapseAIClient` — без Gateway, фронтенда, nginx или Docker — и при этом учитывает те же настройки `config.yaml`, checkpointer, skills, memory, MCP и sandbox, что и остальной SynapseAI.

![SynapseAI TUI](docs/tui/tui-preview.svg)

```bash
uv pip install 'SynapseAI-harness[tui]'        # опциональная зависимость 'textual'

SynapseAI                                      # запустить терминальный UI (требуется TTY)
SynapseAI --continue                           # возобновить последний тред
SynapseAI --resume THREAD                      # возобновить тред по id
SynapseAI --print "summarize this repo"        # автономный разовый ответ в stdout
SynapseAI --json  "hello"                       # автономный режим, StreamEvents с разделением новой строкой
```
...
**Примечание: настоятельно рекомендуем развёртывать SynapseAI только в локальной доверенной сети.** Если вам необходимо развёртывание через несколько устройств или сетей, обязательно реализуйте строгие меры безопасности, например:
...
- **Следите за обновлениями**: регулярно отслеживайте обновления безопасности проекта SynapseAI.
...
SynapseAI стоит на плечах open-source сообщества. Спасибо всем проектам и разработчикам, чья работа сделала его возможным.

Отдельная благодарность:

- **[LangChain](https://github.com/langchain-ai/langchain)** — фреймворк для взаимодействия с LLM и построения цепочек.
- **[LangGraph](https://github.com/langchain-ai/langgraph)** — многоагентная оркестрация, на которой держатся сложные воркфлоу SynapseAI.

### Ключевые контрибьюторы

Авторы SynapseAI, без которых проекта бы не было:
