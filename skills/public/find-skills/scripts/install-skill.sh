...
# Find project root by looking for synapse-ai.code-workspace
find_project_root() {
  local dir="$PWD"
  while [[ "$dir" != "/" ]]; do
    if [[ -f "$dir/synapse-ai.code-workspace" ]]; then
      echo "$dir"
      return 0
    fi
    dir="$(dirname "$dir")"
...
if [[ -z "$PROJECT_ROOT" ]]; then
  echo "Error: Could not find project root (synapse-ai.code-workspace not found)"
  exit 1
fi
