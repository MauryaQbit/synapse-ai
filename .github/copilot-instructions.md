# Copilot Onboarding Instructions for SynapseAI
...
SynapseAI is a full-stack "super agent harness".
...
- `backend/packages/harness/SynapseAI/agents/` - lead agent, middleware chain, memory
- `backend/app/gateway/` - FastAPI gateway API
- `backend/packages/harness/SynapseAI/sandbox/` - sandbox provider + tool wrappers
- `backend/packages/harness/SynapseAI/subagents/` - subagent registry/execution
- `backend/packages/harness/SynapseAI/mcp/` - MCP integration
- `backend/langgraph.json` - graph entrypoint (`SynapseAI.agents:make_lead_agent`)
