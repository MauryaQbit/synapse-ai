...
**Goal:** Add optional Langfuse observability support to SynapseAI while preserving existing LangSmith tracing and allowing both providers to be enabled at the same time.
...
Run: `cd backend && uv run ruff check packages/harness/SynapseAI/config/tracing_config.py packages/harness/SynapseAI/models/factory.py packages/harness/SynapseAI/tracing`
