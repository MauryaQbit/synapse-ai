...
from synapse.agents.lead_agent.agent import build_middlewares
from synapse.config import get_app_config
from synapse.sandbox.middleware import SandboxMiddleware

from synapse.agents.middlewares.thread_data_middleware import ThreadDataMiddleware

HANDSHAKE_ERROR = "[SSL: UNEXPECTED_EOF_WHILE_READING] EOF occurred in violation of protocol (_ssl.c:1000)"
logging.getLogger("synapse.agents.middlewares.tool_error_handling_middleware").setLevel(logging.CRITICAL)
...
def _build_sub_middlewares():
    try:
        from synapse.agents.middlewares.tool_error_handling_middleware import build_subagent_runtime_middlewares
    except Exception:
        return [
