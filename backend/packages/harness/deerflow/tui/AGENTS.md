### Terminal Workbench / TUI (`packages/harness/SynapseAI/tui/`)

A terminal-native UI over the embedded harness, exposed as the `SynapseAI` console script (`[project.scripts]` in `packages/harness/pyproject.toml`). It is a UI shell over `SynapseAIClient` and does **not** fork agent behavior. `textual` is an optional dependency (`SynapseAI-harness[tui]`; also in the backend dev group); the console script degrades to headless help when it is absent. Full guide: [docs/TUI.md](../../../../docs/TUI.md).

**Module layout** (all layers except `app.py` are pure / Textual-free and unit-tested directly):
- `cli.py` — `plan_launch()` (pure launch-mode decision) + headless `--print` / `--json` + `main()` entry point. TTY → TUI, else headless help. `--tui-transparent` / `SYNAPSE_TUI_TRANSPARENT` opt into terminal-default backgrounds without changing the solid-theme default. Uses an **absolute** `from SynapseAI.tui.app import run_tui` so the `app.py` module name doesn't trip `test_harness_boundary.py` (which records relative import module names verbatim).
