...
class MemoryMiddleware(AgentMiddleware[MemoryMiddlewareState]):
    ...
    def _resolve_add_args(self, state: MemoryMiddlewareState, runtime: Runtime) -> tuple[str, list, str, str | None] | None:
        ...
        trace_id = normalize_trace_id(runtime_context.get(SynapseAI_TRACE_METADATA_KEY))
        if trace_id is None:
            try:
                config_data = get_config()
            except RuntimeError:
            ...
            trace_id = normalize_trace_id(config_metadata.get(SynapseAI_TRACE_METADATA_KEY))
        if trace_id is None:
