"""Pluggable memory for SynapseAI.
...
set ``MemoryConfig.manager_class`` -- nothing else in synapse-ai changes.
...
them directly from ``SynapseAI.agents.memory.backends.deermem.deermem.core.*``.
"""
