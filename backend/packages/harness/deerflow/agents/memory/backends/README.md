...
Each subfolder under `agents/memory/backends/` is a pluggable memory backend. Swap the active one by changing one line in `config.yaml` - no synapse-ai core changes required.

- `deermem/` - the default backend (synapse-ai's own: structured facts + JSON storage).
...
| File | What to change |
|---|---|
| `backends/<yourname>/config.py` | Declare your config fields + `from_backend_config` (parse `backend_config`; read `storage_path` from it - **do not import synapse-ai path helpers**) |
...
Then **restart synapse-ai** - the memory manager is a process-level singleton; a running process does not hot-reload config or backend code.
...
> [!IMPORTANT]
> A backend talks to the host through exactly **two channels**: (1) the ABC method arguments (`manager.py`), and (2) the `backend_config` dict. The **only** `from SynapseAI` import allowed anywhere in your backend folder is the ABC contract line in `<name>_manager.py`:

```python
from SynapseAI.agents.memory.manager import MemoryManager
```

Change that one line (and only that line) to port the backend to another agent. **Do not import synapse-ai path helpers, config singletons, or models** - get `storage_path` and everything else from `backend_config`.
...
| File | Role |
|---|---|
| `packages/harness/SynapseAI/agents/memory/manager.py` | ABC + factory + scanner |
| `packages/harness/SynapseAI/agents/middlewares/memory_middleware.py` | `after_agent` -> `manager.add` |
| `packages/harness/SynapseAI/agents/memory/summarization_hook.py` | summarization -> `manager.add_nowait` |
| `packages/harness/SynapseAI/agents/lead_agent/prompt.py` | `_get_memory_context` -> `manager.get_context` |
| `app/gateway/routers/memory.py` | HTTP endpoints -> `manager.*` (direct call + try/except `NotImplementedError`) |
| `packages/harness/SynapseAI/config/memory_config.py` | shared 4 fields (`enabled` / `injection_enabled` / `manager_class` / `backend_config`) |
...
4. **Don't import `runtime_home`.** Read `storage_path` from `backend_config`. (The `noop` template shows the correct pattern; importing synapse-ai path helpers breaks portability - contract #4.)
5. **Restart synapse-ai after changes.** The manager is a process-level singleton; a running process does not hot-reload config or backend code.
...
| `workspace_prefix` | str | `SynapseAI-u-` | Prefix for isolated workspaces; each user gets one workspace named `{prefix}{sanitized_id}` |
...
| `assistant_peer` | str | `SynapseAI` | Default peer name for the assistant when storing messages |
...
- **Contract + factory**: `packages/harness/SynapseAI/agents/memory/manager.py` (`MemoryManager` base, `MemoryCallbacks`, `get_memory_manager` factory).
