...
Runs each SynapseAI sandbox as a [BoxLite](https://github.com/boxlite-ai/boxlite)
...
The `boxlite` package is an optional SynapseAI harness extra, not part of the
...
hosts, such as Windows, should use another sandbox provider or run SynapseAI from
a supported Linux/macOS environment.

**Host requirement:** BoxLite boots micro-VMs, so a Linux host needs KVM — i.e.
nested virtualization when SynapseAI runs inside a cloud VM. macOS uses
...
inside the box and reuse `SynapseAI.sandbox.search`, mirroring `e2b_sandbox`:
