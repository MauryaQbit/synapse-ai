...
deliberately kept as a sibling to :mod:`SynapseAI.guardrails` rather than folded
...
2. **Run-time execution deny** — reuses :class:`~SynapseAI.guardrails.middleware.GuardrailMiddleware`
   via a thin adapter (see :mod:`SynapseAI.authz.adapter`), catching dynamic
...
class AuthorizationProvider(Protocol):
    ...
    mechanism SynapseAI uses for models, tools, sandbox, and guardrails.
