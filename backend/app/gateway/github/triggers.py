...
def _mentions(body: str, login: str) -> bool:
    ...
    ``@SynapseAI`` would falsely match ``@SynapseAI-bot`` (a different,
    ...
    character (e.g. ``foo@SynapseAI`` inside an email address) to avoid
...
def event_should_fire(
    ...
    if trigger.require_mention:
        ...
        # Boundary-aware @-mention match: ``@SynapseAI`` must NOT match
        # ``@SynapseAI-bot`` (a distinct, legitimate GitHub login). See
