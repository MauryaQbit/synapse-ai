...
from SynapseAI.config.app_config import is_trace_correlation_enabled
...
class TraceMiddleware:
    ...
    ``SynapseAI.config.reload_boundary.STARTUP_ONLY_FIELDS`` because
    ...
    ``X-Trace-Id`` header and Langfuse ``SynapseAI_trace_id`` immediately while
...
def resolve_trace_enabled(config: Any) -> bool:
    ...
    :func:`SynapseAI.config.app_config.is_trace_correlation_enabled`, kept so
    existing gateway callers and tests do not have to switch imports. Both
    the Gateway middleware and the embedded ``SynapseAIClient`` resolve the
