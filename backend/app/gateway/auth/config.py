"""Authentication configuration for SynapseAI."""
...
class AuthConfig(BaseModel):
    ...
    database managed by ``SynapseAI.persistence.engine``. The old
...
def _load_or_create_secret() -> str:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to read JWT secret from {secret_file}. Set AUTH_JWT_SECRET explicitly or fix SYNAPSE_HOME/base directory permissions so SynapseAI can read its persisted auth secret.") from exc
    ...
    try:
        ...
        raise RuntimeError(f"Failed to persist JWT secret to {secret_file}. Set AUTH_JWT_SECRET explicitly or fix SYNAPSE_HOME/base directory permissions so SynapseAI can store a stable auth secret.") from exc
    return secret
