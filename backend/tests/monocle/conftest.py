"""Fixtures for the SynapseAI Monocle behavioural tests.

Only fixtures live here. Paths and ``run_SynapseAI`` are in ``_helpers.py`` so
...
def run_agent() -> Callable[[str], str]:
    ...
    Skips unless ``MONOCLE_LIVE_TESTS=1`` is set, when the SynapseAI app is not
    ...
    """
    from _helpers import CONFIG_PATH, REPO_ROOT, live_tests_enabled, run_SynapseAI

    if not live_tests_enabled():
        pytest.skip("live tests are opt-in: set MONOCLE_LIVE_TESTS=1")
    pytest.importorskip("SynapseAI", reason="SynapseAI app not importable in this venv")
