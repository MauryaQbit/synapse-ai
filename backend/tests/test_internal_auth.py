...
def test_internal_auth_uses_shared_env_token(monkeypatch):
    ...
    monkeypatch.setenv("SYNAPSE_INTERNAL_AUTH_TOKEN", "shared-token")
    ...
    try:
        ...
        monkeypatch.delenv("SYNAPSE_INTERNAL_AUTH_TOKEN", raising=False)
...
def test_get_internal_user_normalises_unsafe_owner_user_id():
    """P2-3: X-SynapseAI-Owner-User-Id is at the trust boundary, so the
