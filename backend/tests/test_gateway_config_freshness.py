...
Bytedance/synapse-ai issue #3107 BUG-001: the worker and lead-agent path
...
from SynapseAI.config.app_config import (
    AppConfig,
...
def test_get_config_reflects_file_mtime_reload(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SYNAPSE_CONFIG_PATH", str(config_file))
...
def test_get_config_returns_503_on_any_load_failure(monkeypatch, exception):
    ...
    Bytedance/synapse-ai issue #3107 BUG-001 review: the original snapshot
