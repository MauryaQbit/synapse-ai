...
from SynapseAI.runtime import DisconnectMode, RunManager, RunStatus, ThreadOperationKind
from SynapseAI.runtime.events.store.memory import MemoryRunEventStore
from SynapseAI.runtime.runs.manager import CancelOutcome, ConflictError, PersistenceRetryPolicy, RunStartOutcome
...
async def test_completion_persistence_warns_when_recreated_row_still_missing(caplog):
    ...
    record = await manager.create("thread-1")
    await manager.set_status(record.run_id, RunStatus.success)
    caplog.set_level(logging.WARNING, logger="SynapseAI.runtime.runs.manager")
...
async def test_list_by_thread_is_stable_when_timestamps_tie(manager: RunManager, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("SynapseAI.runtime.runs.manager._now_iso", lambda: "2026-01-01T00:00:00+00:00")
