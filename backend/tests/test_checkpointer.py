...
from SynapseAI.config.checkpointer_config import (
    CheckpointerConfig,
...
from SynapseAI.runtime.checkpointer.provider import POSTGRES_INSTALL
from SynapseAI.runtime.store import get_store, reset_store
from SynapseAI.runtime.store.provider import POSTGRES_STORE_INSTALL
...
class TestCheckpointerConfig:
    ...
    def test_postgres_schema_accepts_valid_identifier(self):
        config = CheckpointerConfig(type="postgres", connection_string="postgresql://localhost/db", postgres_schema="SynapseAI")
        assert config.postgres_schema == "SynapseAI"
    ...
    def test_ensure_config_loaded_loads_app_config_when_uninitialized(self):
        ...
        with patch("SynapseAI.config.app_config.get_app_config", side_effect=fake_get_app_config) as mock_get_app_config:
            ensure_config_loaded()
    ...
    def test_ensure_config_loaded_skips_explicit_config(self):
        ...
        with patch("SynapseAI.config.app_config.get_app_config") as mock_get_app_config:
            ensure_config_loaded()
...
class TestHarnessPackaging:
    ...
    def test_workspace_pyproject_forwards_postgres_extra_to_harness(self):
        ...
        optional_dependencies = data["project"]["optional-dependencies"]
        assert optional_dependencies["postgres"] == ["SynapseAI-harness[postgres]"]

    def test_postgres_missing_dependency_messages_recommend_package_extra(self):
        assert "SynapseAI-harness[postgres]" in POSTGRES_INSTALL
        assert "SynapseAI-harness[postgres]" in POSTGRES_STORE_INSTALL
...
class TestGetCheckpointer:
    def test_returns_in_memory_saver_when_not_configured(self):
        ...
        with patch("SynapseAI.runtime.checkpointer.provider.get_app_config", side_effect=FileNotFoundError):
            cp = get_checkpointer()
    ...
    def test_sqlite_creates_parent_dir(self):
        ...
        .synapse-ai directory has not been created).
        """
        ...
        with (
            ...
            patch("SynapseAI.runtime.checkpointer.provider.ensure_sqlite_parent_dir") as mock_ensure,
            patch(
                "SynapseAI.runtime.checkpointer.provider.resolve_sqlite_conn_str",
    ...
    def test_sqlite_ensure_parent_dir_before_connect(self):
        ...
        with (
            patch.dict(sys.modules, {"langgraph.checkpoint.sqlite": mock_module}),
            patch(
                "SynapseAI.runtime.checkpointer.provider.ensure_sqlite_parent_dir",
    ...
    def test_postgres_schema_creates_schema_and_sets_search_path(self):
        ...
        load_checkpointer_config_from_dict({"type": "postgres", "connection_string": "postgresql://localhost/db", "postgres_schema": "SynapseAI"})
        ...
        mock_conn.execute.assert_called_once_with('CREATE SCHEMA IF NOT EXISTS "SynapseAI"')
        ...
        assert "options=-c%20search_path%3DSynapseAI" in called_dsn
        mock_saver_instance.setup.assert_called_once()
...
class TestSyncSingletonThreadSafety:
    ...
    def test_concurrent_checkpointer_getter_creates_one_instance(self):
        ...
        with patch("SynapseAI.runtime.checkpointer.provider._sync_checkpointer_cm", side_effect=factory.context_manager):
    ...
    def test_concurrent_store_getter_creates_one_instance(self):
        ...
        with patch("SynapseAI.runtime.store.provider._sync_store_cm", side_effect=factory.context_manager):
            futures_started = ThreadPoolExecutor(max_workers=1)
            try:
    ...
    def test_checkpointer_loads_config_outside_singleton_lock(self):
        ...
        with (
            patch("SynapseAI.runtime.checkpointer.provider._checkpointer_lock", tracking_lock),
            patch("SynapseAI.runtime.checkpointer.provider.ensure_config_loaded", side_effect=fake_ensure_config_loaded),
        ):
    ...
    def test_store_loads_config_outside_singleton_lock(self):
        ...
        with (
            patch("SynapseAI.runtime.store.provider._store_lock", tracking_lock),
            patch("SynapseAI.runtime.store.provider.ensure_config_loaded", side_effect=fake_ensure_config_loaded),
        ):
            store = get_store()
    ...
    def test_checkpointer_reset_waits_for_initialization(self):
        ...
        with (
            patch("SynapseAI.runtime.checkpointer.provider._sync_checkpointer_cm", side_effect=factory.context_manager),
    ...
    def test_store_reset_waits_for_initialization(self):
        ...
        with (
            patch("SynapseAI.runtime.store.provider._sync_store_cm", side_effect=factory.context_manager),
            ThreadPoolExecutor(max_workers=2) as executor,
        ):
...
class TestAsyncCheckpointer:
    ...
    async def test_sqlite_creates_parent_dir_via_to_thread(self):
        ...
        from SynapseAI.runtime.checkpointer.async_provider import _prepare_sqlite_checkpointer_path, make_checkpointer
        ...
        with (
            patch("SynapseAI.runtime.checkpointer.async_provider.get_app_config", return_value=mock_config),
            patch.dict(sys.modules, {"langgraph.checkpoint.sqlite.aio": mock_module}),
            patch(
                "SynapseAI.runtime.checkpointer.async_provider.asyncio.to_thread",
    ...
    async def test_postgres_uses_connection_pool(self):
        ...
        mock_config = MagicMock()
        mock_config.checkpointer = CheckpointerConfig(type="postgres", connection_string="postgresql://localhost/db", postgres_schema="SynapseAI")
        ...
        assert "options=-c%20search_path%3DSynapseAI" in call_kwargs[0][0]
        assert call_kwargs[1]["check"] is mock_pool_cls.check_connection
        assert "options" not in call_kwargs[1]["kwargs"]
        mock_conn.execute.assert_awaited_once_with('CREATE SCHEMA IF NOT EXISTS "SynapseAI"')
    ...
    async def test_database_postgres_uses_connection_pool(self):
        ...
        db_config = DatabaseConfig(backend="postgres", postgres_url="postgresql://localhost/db", postgres_schema="SynapseAI")
        mock_config = MagicMock()
    ...
    async def test_database_sqlite_creates_parent_dir_via_to_thread(self):
        ...
        from SynapseAI.config.database_config import DatabaseConfig
        from SynapseAI.runtime.checkpointer.async_provider import _prepare_database_sqlite_checkpointer_path, make_checkpointer
        ...
        with (
            ...
            patch(
                "SynapseAI.runtime.checkpointer.async_provider.asyncio.to_thread",
                new_callable=AsyncMock,
                return_value="/tmp/data/SynapseAI.db",
            ) as mock_to_thread,
        ):
            async with make_checkpointer() as saver:
                assert saver is mock_saver
        ...
        mock_saver_cls.from_conn_string.assert_called_once_with("/tmp/data/SynapseAI.db")
        mock_saver.setup.assert_awaited_once()
...
class TestAsyncStore:
    ...
    async def test_database_postgres_schema_creates_schema_and_sets_search_path(self):
        ...
        mock_config = MagicMock()
        mock_config.checkpointer = None
        mock_config.database = DatabaseConfig(backend="postgres", postgres_url="postgresql://localhost/db", postgres_schema="SynapseAI")
...
class TestCheckpointerDatabaseConfig:
    ...
    silently ignoring ``database``. Embedded callers (``SynapseAIClient``) and the
    ...
    def test_sync_checkpointer_context_uses_database_config(self):
        ...
        from SynapseAI.runtime.checkpointer.provider import checkpointer_context
        ...
        with (
            patch("SynapseAI.runtime.checkpointer.provider.get_app_config", return_value=app_config),
            patch("SynapseAI.runtime.checkpointer.provider._sync_checkpointer_cm", factory),
    ...
    def test_sync_checkpointer_singleton_uses_database_config(self):
        ...
        with (
            patch("SynapseAI.runtime.checkpointer.provider.ensure_config_loaded"),
            patch("SynapseAI.runtime.checkpointer.provider.get_app_config", return_value=app_config),
            patch("SynapseAI.runtime.checkpointer.provider._sync_checkpointer_cm", factory),
        ):
    ...
    def test_sync_checkpointer_falls_back_to_memory_when_config_file_is_missing(self):
        ...
        with (
            patch("SynapseAI.runtime.checkpointer.provider.ensure_config_loaded"),
            patch("SynapseAI.runtime.checkpointer.provider.get_checkpointer_config", return_value=None),
            patch("SynapseAI.runtime.checkpointer.provider.get_app_config", side_effect=FileNotFoundError),
        ):
...
class TestStoreDatabaseConfig:
    def test_sync_store_falls_back_to_memory_when_config_file_is_missing(self):
        ...
        with (
            patch("SynapseAI.runtime.store.provider.ensure_config_loaded"),
            patch("SynapseAI.runtime.store.provider.get_checkpointer_config", return_value=None),
            patch("SynapseAI.runtime.store.provider.get_app_config", side_effect=FileNotFoundError),
        ):
    ...
    async def test_async_postgres_store_uses_database_config(self, caplog):
        ...
        caplog.set_level("WARNING", logger="SynapseAI.runtime.store.async_provider")
        app_config = SimpleNamespace(
            checkpointer=None,
            database=DatabaseConfig(backend="postgres", postgres_url="postgresql://localhost/db"),
        )
    ...
    async def test_async_sqlite_store_uses_unified_database_path(self, tmp_path):
        """Unified database SQLite config must use the shared SynapseAI.db path."""
        from SynapseAI.runtime.store.async_provider import make_store
        from SynapseAI.runtime.store.provider import ensure_sqlite_parent_dir
        ...
        with (
            ...
            patch(
                "SynapseAI.runtime.store.async_provider.asyncio.to_thread",
                new_callable=AsyncMock,
            ) as mock_to_thread,
        ):
            async with make_store(app_config) as store:
    ...
    def test_sync_store_context_uses_database_config(self):
        ...
        from SynapseAI.runtime.store.provider import store_context
        ...
        with (
            patch("SynapseAI.runtime.store.provider.get_app_config", return_value=app_config),
            patch("SynapseAI.runtime.store.provider._sync_store_cm", factory),
    ...
    def test_explicit_memory_database_uses_in_memory_store(self):
        ...
        with patch("SynapseAI.runtime.store.provider.get_app_config", return_value=app_config):
...
# ---------------------------------------------------------------------------
# SynapseAIClient falls back to config checkpointer
# ---------------------------------------------------------------------------


class TestClientCheckpointerFallback:
    def test_client_uses_config_checkpointer_when_none_provided(self):
        """SynapseAIClient._ensure_agent falls back to get_checkpointer() when checkpointer=None."""
        from langgraph.checkpoint.memory import InMemorySaver
        ...
        with (
            patch("SynapseAI.client.get_app_config", return_value=config_mock),
            patch("SynapseAI.client.create_agent", side_effect=fake_create_agent),
            patch("SynapseAI.client.create_chat_model", return_value=MagicMock()),
            patch("SynapseAI.client.build_middlewares", return_value=[]),
            patch("SynapseAI.client.apply_prompt_template", return_value=""),
            patch("SynapseAI.client.get_enabled_skills_for_config", return_value=[]),
            patch("SynapseAI.client.SynapseAIClient._get_tools", return_value=[]),
        ):
            client = SynapseAIClient(checkpointer=None)
    ...
    def test_client_explicit_checkpointer_takes_precedence(self):
        ...
        with (
            ...
            client = SynapseAIClient(checkpointer=explicit_cp)
            config = client._get_runnable_config("test-thread")
            client._ensure_agent(config)
