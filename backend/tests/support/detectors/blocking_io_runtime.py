"""Strict Blockbuster runtime context scoped to SynapseAI business code.

Creates a `BlockBuster` instance with `scanned_modules=("app", "SynapseAI")`
...
sync IO whose caller stack passes through `app.*` or `SynapseAI.*` raises
`BlockingError`.
...
_SCANNED_MODULES: tuple[str, ...] = ("app", "SynapseAI")

# Add SynapseAI-local rules here only when Blockbuster's default rule set misses
...
def detect_blocking_io_strict() -> Iterator[BlockBuster]:
    """Activate Blockbuster scoped to app.* and SynapseAI.* callers only."""
