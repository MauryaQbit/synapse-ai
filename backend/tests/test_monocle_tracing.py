...
importing ``SynapseAI.agents`` no longer sets up telemetry at import time.
"""
...
from SynapseAI.config import is_monocle_tracing_enabled
from SynapseAI.config.tracing_config import get_tracing_config, reset_tracing_config
...
def clear_monocle_env(monkeypatch):
    ...
    monkeypatch.setattr("SynapseAI.tracing.monocle._setup_completed", False)
...
def test_toggles_on_and_sets_up(monkeypatch):
    ...
    assert captured == {"workflow_name": "synapse-ai", "monocle_exporters_list": "file"}
...
def test_embedded_hint_when_enabled_but_uninitialized(monkeypatch, caplog):
    ...
    The embedded ``SynapseAIClient`` and the TUI never hit the Gateway lifespan,
    ...
    with caplog.at_level(logging.DEBUG, logger="SynapseAI.tracing.factory"):
...
def test_no_import_time_setup():
    """Regression: importing SynapseAI.agents must not install telemetry.

    The setup call used to live at module import in ``SynapseAI/agents/__init__``.
    ...
    script = textwrap.dedent(
        ...
        import SynapseAI.agents as agents
