...
from SynapseAI.runtime.runs.worker import _build_runtime_context, _install_runtime_context
...
def _patch_update_agent_dependencies(tmp_path: Path):
    ...
    return [
        patch(
            "SynapseAI.tools.builtins.update_agent_tool.get_paths",
            return_value=_make_paths_mock(tmp_path),
        ),
        patch(
            "SynapseAI.tools.builtins.update_agent_tool.get_app_config",
        ...
        patch(
            "SynapseAI.config.agents_config.get_paths",
            return_value=_make_paths_mock(tmp_path),
        ),
    ]
