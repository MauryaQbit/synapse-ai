...
from SynapseAI.agents.memory.backends.deermem.deermem.core.prompt import _coerce_confidence, format_memory_for_injection
...
def test_format_memory_respects_budget_when_adding_facts(monkeypatch) -> None:
    ...
    monkeypatch.setattr("SynapseAI.agents.memory.backends.deermem.deermem.core.prompt._count_tokens", lambda text, encoding_name="cl100k_base", *, use_tiktoken=True: len(text))
...
def test_guaranteed_correction_injected_when_budget_tight(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "SynapseAI.agents.memory.backends.deermem.deermem.core.prompt._count_tokens",
...
def test_fallback_on_ranking_error(monkeypatch) -> None:
    ...
    call_count = {"n": 0}
    prompt_module = __import__("SynapseAI.agents.memory.backends.deermem.deermem.core.prompt", fromlist=["_select_fact_lines"])
    ...
    monkeypatch.setattr(
        "SynapseAI.agents.memory.backends.deermem.deermem.core.prompt._select_fact_lines",
        flaky_select,
    )
...
def test_fallback_uses_prefiltered_valid_facts(monkeypatch) -> None:
    ...
    call_count = {"select": 0}
    original_select = __import__("SynapseAI.agents.memory.backends.deermem.deermem.core.prompt", fromlist=["_select_fact_lines"])._select_fact_lines
    ...
    monkeypatch.setattr("SynapseAI.agents.memory.backends.deermem.deermem.core.prompt._select_fact_lines", raising_select)
