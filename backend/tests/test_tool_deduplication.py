...
@patch("SynapseAI.tools.tools.get_app_config")
@patch("SynapseAI.tools.tools.is_host_bash_allowed", return_value=True)
def test_config_loaded_async_only_tool_gets_sync_wrapper(mock_bash, mock_cfg):
    ...
    with (
        patch("SynapseAI.tools.tools.resolve_variable", return_value=async_tool),
        patch("SynapseAI.tools.tools.BUILTIN_TOOLS", []),
    ):
...
def test_subagent_async_only_tool_gets_sync_wrapper(mock_bash, mock_cfg):
    ...
    with (
        patch("SynapseAI.tools.tools.BUILTIN_TOOLS", []),
        patch("SynapseAI.tools.tools.SUBAGENT_TOOLS", [async_tool]),
    ):
...
def test_acp_async_only_tool_gets_sync_wrapper(mock_bash, mock_cfg):
    ...
    with (
        patch("SynapseAI.tools.tools.BUILTIN_TOOLS", []),
        patch("SynapseAI.tools.builtins.invoke_acp_agent_tool.build_invoke_acp_agent_tool", return_value=async_tool),
    ):
...
def test_no_duplicates_returned(mock_bash, mock_cfg):
    ...
    with patch("SynapseAI.tools.tools.BUILTIN_TOOLS", [_tool_alpha, _tool_alpha_dup, _tool_beta]):
...
def test_first_occurrence_wins(mock_bash, mock_cfg):
    ...
    with patch("SynapseAI.tools.tools.BUILTIN_TOOLS", [sentinel_alpha, sentinel_alpha_dup, _tool_beta]):
        result = get_available_tools(include_mcp=False)
...
def test_duplicate_triggers_warning(mock_bash, mock_cfg, caplog):
    ...
    with patch("SynapseAI.tools.tools.BUILTIN_TOOLS", [_tool_alpha, _tool_alpha_dup]):
        with caplog.at_level(logging.WARNING, logger="SynapseAI.tools.tools"):
