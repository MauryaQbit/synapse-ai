...
def test_official_entrypoints_route_pnpm_through_shared_runner():
    ...
    assert '"$SynapseAI_PNPM_PYTHON" "$SynapseAI_PNPM_RUNNER" install --silent' in serve_script
    assert 'SynapseAI_PNPM_RUNNER="$REPO_ROOT/scripts/pnpm.py"' in serve_script
    assert 'FRONTEND_CMD=\'env PORT=3000 "$SynapseAI_PNPM_PYTHON" "$SynapseAI_PNPM_RUNNER" run dev\'' in serve_script
    assert '"\\$SynapseAI_PNPM_RUNNER\\" run preview"' in serve_script
