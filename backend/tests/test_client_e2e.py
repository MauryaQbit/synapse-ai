"""End-to-end tests for SynapseAIClient.
...
Only SYNAPSE_HOME is redirected to tmp_path for filesystem isolation.
...
def e2e_env(tmp_path, monkeypatch):
    ...
    - SYNAPSE_HOME → tmp_path (all thread data lands in a temp dir)
    - SYNAPSE_PROJECT_ROOT → repository root (shared skills/config assets
    ...
    monkeypatch.setattr("SynapseAI.sandbox.sandbox_provider._default_sandbox_provider", None)
    ...
    monkeypatch.setattr("SynapseAI.config.app_config._app_config", None)
    monkeypatch.setattr("SynapseAI.config.app_config._app_config_is_custom", False)
    config = _make_e2e_config()
    monkeypatch.setattr("SynapseAI.config.app_config._app_config", config)
    monkeypatch.setattr("SynapseAI.config.app_config._app_config_is_custom", True)
    monkeypatch.setattr("SynapseAI.client.get_app_config", lambda: config)

    # 3. Disable title generation (extra LLM call, non-deterministic)
    from SynapseAI.config.title_config import TitleConfig

    monkeypatch.setattr("SynapseAI.config.title_config._title_config", TitleConfig(enabled=False))

    # 4. Disable memory queueing (avoids background threads & file writes)
    from SynapseAI.config.memory_config import MemoryConfig

    monkeypatch.setattr(
        "SynapseAI.agents.middlewares.memory_middleware.get_memory_config",
    ...
    monkeypatch.setattr("SynapseAI.config.summarization_config._summarization_config", SummarizationConfig(enabled=False))
    ...
    from SynapseAI.agents.lead_agent.agent import build_middlewares as _original_build_middlewares
    ...
    monkeypatch.setattr("SynapseAI.client.build_middlewares", _sync_safe_build_middlewares)
...
@pytest.fixture()
def client(e2e_env):
    """A SynapseAIClient wired to the isolated e2e_env."""
    return SynapseAIClient(checkpointer=None, thinking_enabled=False)
...
class TestFileUploadIntegration:
    ...
    def test_upload_files(self, e2e_env, tmp_path):
        ...
        c = SynapseAIClient(checkpointer=None, thinking_enabled=False)
        tid = str(uuid.uuid4())
...
class TestLifecycleAndConfig:
    ...
    def test_plan_mode_config_key(self, e2e_env):
        ...
        c = SynapseAIClient(checkpointer=None, plan_mode=False)
        ...
        c2 = SynapseAIClient(checkpointer=None, plan_mode=True)
...
class TestSkillInstallation:
    ...
    def _isolate_skills_dir(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "SynapseAI.skills.storage._default_skill_storage",
            local_storage,
        )
        monkeypatch.setattr(
            "SynapseAI.client.get_or_new_user_skill_storage",
...
class TestConfigManagement:
    ...
    def test_update_mcp_config_writes_and_invalidates(self, e2e_env, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SYNAPSE_EXTENSIONS_CONFIG_PATH", str(config_file))

        # Force reload so the singleton picks up our test file
        from SynapseAI.config.extensions_config import reload_extensions_config
