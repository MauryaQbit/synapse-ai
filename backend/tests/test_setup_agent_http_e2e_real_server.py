...
def isolated_deer_flow_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    """Stand up an isolated SynapseAI data root + config under tmp_path.

    - Sets ``SYNAPSE_HOME`` so paths land under tmp_path, not the real
      ``.synapse-ai`` directory.
    - Stages a copy of the project's ``config.yaml`` (or ``config.example.yaml``
      ...
      ``SYNAPSE_CONFIG_PATH`` to it, so lifespan boot doesn't depend on the
    ...
    # config to tmp_path and pin ``SYNAPSE_CONFIG_PATH`` to it.
    staged_config = tmp_path / "config.yaml"
    staged_config.write_text(_MINIMAL_CONFIG_YAML, encoding="utf-8")
    monkeypatch.setenv("SYNAPSE_CONFIG_PATH", str(staged_config))
...
def _reset_process_singletons(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    so they pick up our test-only ``SYNAPSE_HOME`` and sqlite path:

    - ``SynapseAI.config.app_config`` caches the parsed ``config.yaml``.
    - ``SynapseAI.config.paths`` caches the ``Paths`` singleton derived from
      ``SYNAPSE_HOME`` at first access.
    - ``SynapseAI.persistence.engine`` caches the SQLAlchemy engine and
...
def isolated_app(isolated_deer_flow_home: Path, monkeypatch: pytest.MonkeyPatch):
    """Build a fresh FastAPI app inside a clean SYNAPSE_HOME.
    ...
    # Re-resolve the config from the test-only SYNAPSE_HOME and pin its
...
def test_real_http_create_agent_lands_in_authenticated_user_dir(
    ...
    """
    # ``SynapseAI.agents.lead_agent.agent`` imports ``create_chat_model`` with
    # ``from SynapseAI.models import create_chat_model`` at module load time,
    ...
    # ``SynapseAI.models.create_chat_model`` would be too late.
    ...
    with (
        patch(
            "SynapseAI.agents.lead_agent.agent.create_chat_model",
