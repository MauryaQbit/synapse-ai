...
from SynapseAI.community.crawl4ai.crawl4ai_client import Crawl4AiClient
...
class TestCrawl4AiClient:
    ...
    async def test_fetch_markdown_success(self):
        with patch("SynapseAI.community.crawl4ai.crawl4ai_client.httpx.AsyncClient") as mock_cls:
            mock_ctx = MagicMock()
...
class TestCrawl4AiTools:
    ...
    @patch("SynapseAI.community.crawl4ai.tools._build_client")
    async def test_web_fetch_tool_success(self, mock_build):
        from SynapseAI.community.crawl4ai import tools

        mock_client = MagicMock()
        mock_client.fetch_markdown = AsyncMock(return_value="# Title\n\nContent")
        mock_build.return_value = mock_client

        with patch("SynapseAI.community.crawl4ai.tools._get_tool_config", return_value=None):
    ...
    async def test_web_fetch_tool_rejects_dns_resolving_to_private(self, mock_build):
        ...
        with patch("SynapseAI.community.crawl4ai.tools._get_tool_config", return_value=None):
            with patch(
                "SynapseAI.community.url_safety.resolve_host_addresses",
    ...
    async def test_web_fetch_tool_allows_private_when_opted_in(self, mock_build):
        ...
        with patch("SynapseAI.community.crawl4ai.tools._get_tool_config", return_value={"allow_private_addresses": True}):
    ...
    async def test_web_fetch_tool_reads_config_once(self, mock_build):
        ...
        with patch("SynapseAI.community.crawl4ai.tools._get_tool_config", return_value={}) as mock_cfg:
    ...
    async def test_web_fetch_tool_passes_configured_filter(self, mock_build):
        ...
        with patch("SynapseAI.community.crawl4ai.tools._get_tool_config", return_value={"filter": "raw"}):
            await tools.web_fetch_tool.ainvoke("https://example.com")
    ...
    async def test_web_fetch_tool_invalid_filter_falls_back_to_fit(self, mock_build):
        ...
        with patch("SynapseAI.community.crawl4ai.tools._get_tool_config", return_value={"filter": "BOGUS"}):
            await tools.web_fetch_tool.ainvoke("https://example.com")
