...
from SynapseAI.persistence.postgres_schema import (
...
class TestBuildAsyncpgConnectArgs:
    def test_sets_search_path_for_schema(self):
        assert build_asyncpg_connect_args("SynapseAI") == {"server_settings": {"search_path": "SynapseAI"}}
...
class TestBuildPsycopgOptions:
    def test_builds_libpq_options(self):
        assert build_psycopg_options("SynapseAI") == "-c search_path=SynapseAI"
...
class TestCreateSchemaSql:
    def test_builds_create_schema_statement(self):
        assert create_schema_sql("SynapseAI") == 'CREATE SCHEMA IF NOT EXISTS "SynapseAI"'

    def test_empty_schema_returns_none(self):
        assert create_schema_sql("") is None

    @pytest.mark.parametrize("schema", ['a"; DROP SCHEMA public; --', "MySchema", "a b", "SynapseAI\n"])
...
class TestDsnWithSearchPath:
    ...
    def test_appends_options_query_encoded(self):
        dsn = "postgresql://u:p@h:5432/db"
        out = dsn_with_search_path(dsn, "SynapseAI")
        # libpq only decodes %XX in URI query values; '+' is NOT treated as a
        # space. The space MUST therefore be encoded as %20, never as '+'.
        assert "+" not in out
        assert "options=-c%20search_path%3DSynapseAI" in out
        parts = urlsplit(out)
        query = parse_qs(parts.query)
        assert query["options"] == ["-c search_path=SynapseAI"]
    ...
    def test_preserves_existing_options_query(self):
        ...
        out = dsn_with_search_path(dsn, "SynapseAI")
        query = parse_qs(urlsplit(out).query)
        assert query["options"] == ["-c statement_timeout=5000 -c search_path=SynapseAI"]
    ...
    def test_supports_keyword_dsn(self):
        ...
        dsn = "host=localhost dbname=SynapseAI user=postgres"
        out = dsn_with_search_path(dsn, "SynapseAI")
        assert conninfo_to_dict(out) == {
            "host": "localhost",
            "dbname": "SynapseAI",
            "user": "postgres",
            "options": "-c search_path=SynapseAI",
        }
    ...
    def test_preserves_keyword_dsn_options(self):
        ...
        dsn = "host=localhost dbname=SynapseAI options='-c statement_timeout=5000'"
        out = dsn_with_search_path(dsn, "SynapseAI")
        assert conninfo_to_dict(out)["options"] == "-c statement_timeout=5000 -c search_path=SynapseAI"
    ...
    def test_rejects_non_postgres_url_scheme(self):
        try:
            dsn_with_search_path("mysql://localhost/db", "SynapseAI")
    ...
    def test_preserves_option_value_containing_space(self):
        ...
        from SynapseAI.persistence.postgres_schema import _merge_search_path_option

        merged = _merge_search_path_option(r"-c application_name=My\ App", "SynapseAI")
        assert "'" not in merged
        assert r"application_name=My\ App" in merged
        assert merged.endswith("-c search_path=SynapseAI")
    ...
    def test_preserves_option_value_containing_tab(self):
        ...
        merged = _merge_search_path_option("-c application_name=My\\\tApp", "SynapseAI")
        assert "'" not in merged
...
class TestNormalizeLibpqDsn:
    ...
    def test_leaves_keyword_dsn_unchanged(self):
        dsn = "host=localhost dbname=SynapseAI"
        assert normalize_libpq_dsn(dsn) == dsn
