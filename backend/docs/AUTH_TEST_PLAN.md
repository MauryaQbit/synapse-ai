...
```bash
# 清除已有数据
rm -f backend/.synapse-ai/data/SynapseAI.db
...
# 从 .synapse-ai/admin_initial_credentials.txt 读取 reset 后密码
...
```bash
# 检查数据库
sqlite3 backend/.synapse-ai/data/SynapseAI.db "SELECT email, password_hash FROM users LIMIT 3;"
```
...
2. 从 `.synapse-ai/admin_initial_credentials.txt` 读取新密码并登录
...
#### TC-UPG-05: 无 SynapseAI.db 时创建 schema 但不创建默认用户

```bash
ls -la backend/.synapse-ai/data/SynapseAI.db
sqlite3 backend/.synapse-ai/data/SynapseAI.db "SELECT COUNT(*) FROM users;"
```

**预期：** 文件存在，`sqlite3` 可查到 `users` 表含 `needs_setup`、`token_version` 列；未调用 `/initialize` 前用户数为 0

#### TC-UPG-06: SynapseAI.db WAL 模式

```bash
sqlite3 backend/.synapse-ai/data/SynapseAI.db "PRAGMA journal_mode;"
```
...
**预期：**
- [ ] 服务正常启动（忽略 `SynapseAI.db`，无 auth 相关代码不报错）
- [ ] 旧对话数据仍然可访问
- [ ] `SynapseAI.db` 文件残留但不影响运行
...
**预期：**
- [ ] 识别已有 `SynapseAI.db`，不重新创建 admin
- [ ] 旧的 admin 账号仍可登录（如果回退期间未删 `SynapseAI.db`）
...
```bash
python -m app.gateway.auth.reset_admin --email admin@example.com
ls -la backend/.synapse-ai/admin_initial_credentials.txt
cat backend/.synapse-ai/admin_initial_credentials.txt
```
...
# 检查 admin 数量
sqlite3 backend/.synapse-ai/data/SynapseAI.db \
...
cp .synapse-ai/admin_initial_credentials.txt /tmp/SynapseAI-reset-p1.txt
P1=$(awk -F': ' '/^password:/ {print $2}' /tmp/SynapseAI-reset-p1.txt)

python -m app.gateway.auth.reset_admin
cp .synapse-ai/admin_initial_credentials.txt /tmp/SynapseAI-reset-p2.txt
P2=$(awk -F': ' '/^password:/ {print $2}' /tmp/SynapseAI-reset-p2.txt)
```

**预期：**
- [ ] `.synapse-ai/admin_initial_credentials.txt` 每次都会被重写，文件权限为 `0600`
- [ ] P1 ≠ P2（每次生成新随机密码）
- [ ] P1 不可用，只有 P2 有效
- [ ] `token_version` 递增了 2
- [ ] `needs_setup` 为 True
...
> - `SYNAPSE_HOME` 挂载到宿主机目录（持久化 `SynapseAI.db`）

#### TC-DOCKER-01: SynapseAI.db 通过 volume 持久化
...
# 检查宿主机上的 SynapseAI.db
ls -la ${SYNAPSE_HOME:-backend/.synapse-ai}/data/SynapseAI.db
sqlite3 ${SYNAPSE_HOME:-backend/.synapse-ai}/data/SynapseAI.db \
  "SELECT email FROM users WHERE email='docker-test@example.com';"
```

**预期：** SynapseAI.db 在宿主机 `SYNAPSE_HOME` 目录中，查询可见刚注册的用户。
...
docker logs synapse-ai-gateway 2>&1 | grep -E "ChannelManager|channel" | head -10
```
...
```bash
# 首次启动不会自动生成 admin 密码。先重置已有 admin，凭据文件写在挂载到宿主机的 SYNAPSE_HOME 下。
docker exec synapse-ai-gateway python -m app.gateway.auth.reset_admin --email docker-test@example.com

ls -la ${SYNAPSE_HOME:-backend/.synapse-ai}/admin_initial_credentials.txt
# 预期文件权限: -rw------- (0600)

cat ${SYNAPSE_HOME:-backend/.synapse-ai}/admin_initial_credentials.txt
# 预期内容: email + password 行

# 容器日志只输出文件路径，不输出密码本身
docker logs synapse-ai-gateway 2>&1 | grep -E "Credentials written to|Admin account"
# 预期看到: "Credentials written to: /...../admin_initial_credentials.txt (mode 0600)"

# 反向验证: 日志里 NEVER 出现明文密码
docker logs synapse-ai-gateway 2>&1 | grep -iE "Password: .{15,}" && echo "FAIL: leaked" || echo "OK: not leaked"
```

**预期：**
- 凭证文件存在于 `SYNAPSE_HOME` 下，权限 `0600`
...
# 确认 gateway 容器存在
docker ps --filter name=synapse-ai-gateway --format '{{.Names}}'
# 预期: synapse-ai-gateway
