...
Gateway (`app/gateway/routers/skills.py`, `uploads.py`) and Client (`SynapseAI/client.py`) each independently implement the same business logic:
...
app.gateway.routers.uploads ──┤── calls ──→  SynapseAI.skills.installer
SynapseAI.client             ──┘              SynapseAI.uploads.manager
```

- Shared modules live in the harness layer (`SynapseAI.*`), pure business logic, no FastAPI dependency
...
### 3.1 `SynapseAI.skills.installer`
...
| Alternative | Why Not |
|-------------|---------|
| Keep logic in Gateway, Client calls Gateway via HTTP | Adds network dependency to embedded Client; defeats the purpose of `SynapseAIClient` as an in-process API |
...
**None.** All public APIs (Gateway HTTP endpoints, `SynapseAIClient` methods) retain their existing signatures and return formats. The `SkillAlreadyExistsError` is a subclass of `ValueError`, so existing `except ValueError` handlers still catch it.
