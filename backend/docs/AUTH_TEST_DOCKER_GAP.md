...
|---|---|---|---|
| TC-DOCKER-01 | `SynapseAI.db` volume persistence | Verify the `SYNAPSE_HOME` bind mount survives container restart | needs `docker compose up` |
...
| TC-DOCKER-05 (credential surfacing) | `reset_admin` writes `.synapse-ai/admin_initial_credentials.txt` with mode 0600 and logs only the path — the only Docker-unique step is whether the bind mount projects this path onto the host, which is a `docker compose` config check, not a runtime behavior change |
...
# Optional: pin SYNAPSE_HOME to a stable host path
echo "SYNAPSE_HOME=$HOME/synapse-ai-data" >> .env
```
