# RFC: `create_SynapseAI_agent` — 纯参数的 SDK 工厂 API
...
**6 处隐式 I/O** — 全部依赖文件系统。如果你想把 `SynapseAI-harness` 当 Python 库嵌入自己的应用，你必须准备 `config.yaml` + `extensions_config.json` + skills 目录。这对 SDK 用户是不可接受的。

### 对比

| | `langchain.create_agent` | `make_lead_agent` | `SynapseAIClient`（增强后） |
|---|---|---|---|
...
| 适合谁 | 写 LangChain 的人 | 内部使用 | **所有 SynapseAI 用户** |
...
```
    ┌──────────────────────┐
    │   SynapseAIClient     │  ← 唯一公开 API（chat/stream + 管理）
    └──────────┬───────────┘
    ┌──────────▼───────────┐
    │   make_lead_agent    │  ← 内部：配置驱动工厂
    └──────────┬───────────┘
    ┌──────────▼───────────┐
    │  create_SynapseAI_agent   │  ← 内部：纯参数工厂
    └──────────┬───────────┘
    ┌──────────▼───────────┐
...
`SynapseAIClient` 是唯一公开 API。`create_SynapseAI_agent` 和 `make_lead_agent` 都是内部实现。

用户通过 `SynapseAIClient` 三个参数控制行为：
...
- **向前兼容** — 现有 `SynapseAIClient()` 无参构造行为不变
- **harness 边界合规** — 不 import `app.*`（`test_harness_boundary.py` 强制）

## 3. API 设计

### 3.1 `SynapseAIClient` — 唯一公开 API
...
# 用法 2：只改参数，不换实现
client = SynapseAIClient(config={"memory": {"max_facts": 50}})

# 用法 3：替换 middleware 实现
client = SynapseAIClient(features=RuntimeFeatures(auto_title=MyTitleMiddleware()))

# 用法 4：添加自定义 middleware
client = SynapseAIClient(extra_middleware=[MyAuditMiddleware()])

# 用法 5：纯 SDK（无 config.yaml）
client = SynapseAIClient(config={
    "models": [{"name": "gpt-4o", "use": "langchain_openai:ChatOpenAI", ...}],
    "tools": [{"name": "bash", "use": "SynapseAI.sandbox.tools:bash_tool", "group": "bash"}],
    "memory": {"enabled": True},
})
```
...
### 3.2 `create_SynapseAI_agent` — 内部工厂（不公开）
...
client = SynapseAIClient(features=RuntimeFeatures(memory=MyMemoryMiddleware()))
...
```python
from SynapseAI.agents import Next, Prev
...
client = SynapseAIClient(config={
    "models": [{"name": "gpt-4o", "use": "langchain_openai:ChatOpenAI", "model": "gpt-4o", "api_key": "sk-..."}],
    "tools": [
        {"name": "bash", "group": "bash", "use": "SynapseAI.sandbox.tools:bash_tool"},
        {"name": "web_search", "group": "web", "use": "SynapseAI.community.tavily.tools:web_search_tool"},
    ],
...
```
Phase 1（当前 PR #1203）:
  ✓ 新增 create_SynapseAI_agent + RuntimeFeatures（内部 API）
  ✓ 不改 SynapseAIClient 和 make_lead_agent
  ✗ middleware 内部仍读 config（已知限制）

Phase 2（#1380）:
  - SynapseAIClient 构造函数增加可选参数（model, tools, features, system_prompt）
  - Options 参数覆盖 config（MemoryOptions, TitleOptions 等）
  - @Next/@Prev 装饰器
  - 补缺失 middleware（Guardrail, TokenUsage, DeferredToolFilter）
  - make_lead_agent 改为薄壳调 create_SynapseAI_agent

Phase 3:
  - SDK 文档和示例
  - SynapseAI.client 稳定 API
```
...
| 问题 | 决议 | 理由 |
|------|------|------|
| 公开 API | `SynapseAIClient` 唯一入口 | 自顶向下，先改现有 API 再抽底层 |
| create_SynapseAI_agent | 内部实现，不公开 | 用户不需要接触 CompiledStateGraph |
