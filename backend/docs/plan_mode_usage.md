...
This document describes how to enable and use the Plan Mode feature with TodoList middleware in SynapseAI 2.0.
...
### Agent Module
- **Location**: `packages/harness/SynapseAI/agents/lead_agent/agent.py`
...
SynapseAI uses custom `system_prompt` and `tool_description` for the TodoListMiddleware that match the overall SynapseAI prompt style:

### System Prompt Features
- Uses XML tags (`<todo_list_system>`) for structure consistency with SynapseAI's main prompt
...
The custom prompts are defined in `_create_todo_list_middleware()` in `/Users/hetao/workspace/synapse-ai/backend/packages/harness/SynapseAI/agents/lead_agent/agent.py:57`.

## Notes

- TodoList middleware uses LangChain's built-in `TodoListMiddleware` with **custom SynapseAI-style prompts**
...
- Custom prompts emphasize the same principles as SynapseAI's main system prompt (clarity, action-oriented, critical rules)
