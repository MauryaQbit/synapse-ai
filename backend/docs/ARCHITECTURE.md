...
This document provides a comprehensive overview of the SynapseAI backend architecture.
...
**Entry Point**: `packages/harness/SynapseAI/agents/lead_agent/agent.py:make_lead_agent`
...
```json
{
  "agent": {
    "type": "agent",
    "path": "SynapseAI.agents:make_lead_agent"
  }
}
```
...
- `threads.py` - `/api/threads/{id}` - Local SynapseAI thread data cleanup after LangGraph deletion
- `artifacts.py` - `/api/threads/{id}/artifacts` - Artifact serving
- `suggestions.py` - `/api/threads/{id}/suggestions` - Follow-up suggestion generation

The web conversation delete flow first deletes Gateway-managed thread state through the LangGraph-compatible route, then the Gateway `threads.py` router removes SynapseAI-managed filesystem data via `Paths.delete_thread_dir()`.
...
│  (packages/harness/SynapseAI/sandbox/local.py) │              │  (packages/harness/SynapseAI/community/)       │
│                         │              │                         │
...
|-------------|---------------|
| `/mnt/user-data/workspace` | `backend/.synapse-ai/threads/{thread_id}/user-data/workspace` |
| `/mnt/user-data/uploads` | `backend/.synapse-ai/threads/{thread_id}/user-data/uploads` |
| `/mnt/user-data/outputs` | `backend/.synapse-ai/threads/{thread_id}/user-data/outputs` |
| `/mnt/skills` | `synapse-ai/skills/` |
...
│  (packages/harness/SynapseAI/tools/)       │  │  (config.yaml)      │  │  (extensions.json)  │
├─────────────────────┤  ├─────────────────────┤  ├─────────────────────┤
...
└─────────────────────┘  └─────────────────────┘  └─────────────────────┘
           ...
           └───────────────────────┴───────────────────────┘
                      ...
                      ┌─────────────────────────┐
                      │   get_available_tools() │
                      │   (packages/harness/SynapseAI/tools/__init__)  │
                      └─────────────────────────┘
```
...
┌─────────────────────────────────────────────────────────────────────────┐
│                          Model Factory                                   │
│                     (packages/harness/SynapseAI/models/factory.py)                              │
└─────────────────────────────────────────────────────────────────────────┘
...
┌─────────────────────────────────────────────────────────────────────────┐
│                          MCP Integration                                 │
│                        (packages/harness/SynapseAI/mcp/manager.py)                              │
└─────────────────────────────────────────────────────────────────────────┘
...
┌─────────────────────────────────────────────────────────────────────────┐
│                          Skills System                                   │
│                       (packages/harness/SynapseAI/skills/loader.py)                             │
└─────────────────────────────────────────────────────────────────────────┘
...
2. Gateway receives file
   - Validates file
   - Stores in .synapse-ai/threads/{thread_id}/user-data/uploads/
...
3. Returns response
   {
     "files": [{
       "filename": "doc.pdf",
       "path": ".synapse-ai/.../uploads/doc.pdf",
...
3. Gateway removes local SynapseAI-managed files
   - Deletes .synapse-ai/threads/{thread_id}/ recursively
